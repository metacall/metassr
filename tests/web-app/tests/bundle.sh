...
# cache/pages/*/index.js          - client hydrator sources
# cache/pages/*/index.server.js   - server render sources
# server/pages/*.js               - esbuild server bundles (flat, one per route)
# pages/*/index.js.js             - esbuild client bundles
EXPECTED_PATTERNS=(
...
check_pattern() {
    ...
    local found_files
    found_files=$(find "$DIST_DIR" -path "$DIST_DIR/$pattern" 2>/dev/null | wc -l)
...
if [ -f "$DIST_DIR/server/head.js" ]; then
    check_file_size "$DIST_DIR/server/head.js" "$MIN_BUNDLE_SIZE" "Head bundle"
fi

FIRST_PAGE_JS=$(find "$DIST_DIR/pages" -name "index.js.js" | head -1)
if [ -n "$FIRST_PAGE_JS" ]; then
    check_file_size "$FIRST_PAGE_JS" "$MIN_BUNDLE_SIZE" "Client page bundle"
fi

FIRST_SERVER_JS=$(find "$DIST_DIR/server/pages" -name "*.js" | head -1)
if [ -n "$FIRST_SERVER_JS" ]; then
    check_file_size "$FIRST_SERVER_JS" "$MIN_BUNDLE_SIZE" "Server page bundle"
fi
